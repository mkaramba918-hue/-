...
# ID вашего сервера для мгновенной синхронизации слеш-команд
...
async def send_log(guild, message):
    ...
    if row:
        ...
        if log_channel:
            try: await log_channel.send(message)
            except: pass
...
class RoleSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cursor.execute('SELECT points FROM users WHERE user_id = ?', (interaction.user.id,))
        user_points = cursor.fetchone()[0] if cursor.fetchone() else 0
        
        if user_points < price:
            await interaction.followup.send("❌ Недостаточно монет!", ephemeral=True)
        else:
            cursor.execute('UPDATE users SET points = points - ? WHERE user_id = ?', (price, interaction.user.id))
            conn.commit()
            await interaction.user.add_roles(role)
            await interaction.followup.send(f"✅ Роль {role.name} куплена!", ephemeral=True)
            await send_log(interaction.guild, f"🛒 Покупка: {interaction.user.mention} купил {role.name}")
        conn.close()
...
class RoleShopView(discord.ui.View):
    def __init__(self, guild):
        ...
        self.guild = guild
        conn = sqlite3.connect('economy.db')
        items = conn.cursor().execute('SELECT role_id, price FROM shop_roles').fetchall()
        conn.close()
        options = [discord.SelectOption(label=self.guild.get_role(r[0]).name, value=str(r[0])) for r in items if self.guild.get_role(r[0])]
        if options: self.add_item(RoleSelect(options))

# --- Слеш-команда /shop ---
@bot.tree.command(name="shop", description="Открыть магазин")
async def shop(interaction: discord.Interaction):
    view = RoleShopView(interaction.guild)
    if not view.children: await interaction.response.send_message("🛒 Пусто.", ephemeral=True)
